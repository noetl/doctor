...
Out-of-process runtime reaper for NoETL.
...
The playbook's terminal `report` step emits a JSON object, doctor
re-emits that object as a stable `Outcome` shape on stdout, and the
...
doctor/
  ...
  src/
    ...
    runner.rs            # spawn `noetl run`, parse last JSON object
...
- Provisioning playbooks use `kind: shell` with inline `kubectl`/
  `helm` and an `ensure_kube_context` guard so they refuse to run
...
target/release/noetl-doctor detect \
  ...
  --pg-dsn   postgresql://noetl@localhost:5432/noetl \
  --stale-seconds 300
...
`NOETL_CLI_VERSION` / `NOETL_CLI_ARCH` build args. To target a
different release, pass `--build-arg NOETL_CLI_VERSION=2.13.0`.
